...
static int shared_array_type(const Type *t){ return t->array_n>0 && t->as==AS_THREADGROUP; }
static void array_elem_ll(char *buf,size_t n,const Type *t){
    char elem[128];
    if(t->matn) mll_of(elem,sizeof elem,t->matn,t->matm);
    else type_ll(elem,sizeof elem,t->kind,t->struct_name,t->vecn);
    if(t->array_m) snprintf(buf,n,"[%d x %s]",t->array_m,elem);
...
static void expr_type_of(CG *c, Expr *e, Type *out){
    ...
    case E_CALL:{
        ...
        if((!strcmp(e->name,"all")||!strcmp(e->name,"any"))&&e->nargs==1){
...
static const char *gen_composite_math(CG *c, Expr *e, ValKind *k){
    ...
    if(!strcmp(nm,"all")||!strcmp(nm,"any")){
        if(e->nargs!=1) die(0,"%s expects 1 argument",nm);
        if(ak!=VK_I1) die(0,"%s argument must be a bool or bool vector",nm);
        int is_all=!strcmp(nm,"all");
...
static const char *gen_rval(CG *c, Expr *e, ValKind *k){
    ...
    case E_CALL:{
        ...
        if(e->callee && e->callee->kind==E_FIELD){
            ...
            if(e->callee->operand->kind==E_IDENT){
                ...
                if(tr==R_TEXTURE){
                    ...
                    (void)elt; (void)an;
                    char tname[64]; snprintf(tname,sizeof tname,"%%_%s",tp->name);
                    if(!strcmp(e->name,"GetDimensions")){
                        if(tp->ty.tex_dim!=2||tp->ty.tex_array||tp->ty.tex_cube||e->nargs!=4)
                            die(0,"Texture2D.GetDimensions expects (mip, width, height, mipCount)");
                        ValKind mk; const char *mip=gen_rval(c,e->args[0],&mk);
                        if(c->rvw|| (mk!=VK_I32&&mk!=VK_U32)) die(0,"GetDimensions mip must be a scalar integer");
                        if(mk==VK_U32) mip=coerce(c,mip,mk,VK_I32);
                        const char *q[3];
                        for(int oi=0;oi<3;oi++){
                            LInfo li; q[oi]=gen_lval(c,e->args[oi+1],&li,1);
                            if(li.tk!=T_UINT32||li.vecn||li.matn||li.an)
                                die(0,"GetDimensions outputs must be uint lvalues");
                            const char *name=oi==0?"get_width":oi==1?"get_height":"get_num_mip_levels";
                            const char *r=newtmp(c);
                            if(oi<2) emit(c,"  %s = call i32 @air.%s_texture_2d(%%struct._texture_2d_t addrspace(1)* %s, i32 %s)\n",r,name,tname,mip);
                            else emit(c,"  %s = call i32 @air.%s_texture_2d(%%struct._texture_2d_t addrspace(1)* %s)\n",r,name,tname);
                            emit(c,"  store i32 %s, i32* %s, align 4\n",r,q[oi]);
                        }
                        tex_dims_2d_used=1; *k=VK_I32; c->rvw=0; return "0";
                    }
                    ...
                    if(!strcmp(e->name,"SampleGrad")){
                        ...
                        int si; if(e->args[0]->kind!=E_IDENT||resolve(c,e->args[0]->name,&si)!=R_SAMPLER)
                            die(0,"texture SampleGrad's first argument must be a sampler parameter");
                        ValKind uk; const char *uv=gen_rval(c,e->args[1],&uk); int uvw=c->rvw;
                        ValKind dxk; const char *dx=gen_rval(c,e->args[2],&dxk); int dxw=c->rvw;
                        ValKind dyk; const char *dy=gen_rval(c,e->args[3],&dyk); int dyw=c->rvw;
                        if(uk!=VK_F32||dxk!=VK_F32||dyk!=VK_F32||
                           (tp->ty.tex_cube&&tp->ty.tex_array ? (dxw!=3||dyw!=3) :
                            (tp->ty.tex_array&&tp->ty.tex_dim==2 ? (dxw!=2||dyw!=2) : (dxw!=uvw))))
                            die(0,"SampleGrad coordinates and gradients must be equally-sized floats");
                        int ti_kind=tp->ty.tex_elt==T_HALF?1:tp->ty.tex_elt==T_INT32?2:tp->ty.tex_elt==T_UINT32?3:0;
                        const char *r=newtmp(c); char sname[64]; snprintf(sname,sizeof sname,"%%_%s",e->args[0]->name);
                        if(tp->ty.tex_cube&&tp->ty.tex_array){
                            if(uvw!=4) die(0,"cube-array SampleGrad coordinate must be a float4");
                            const char *dir=newtmp(c); emit(c,"  %s = shufflevector <4 x float> %s, <4 x float> undef, <3 x i32> <i32 0, i32 1, i32 2>\n",dir,uv);
                            const char *layerf=newtmp(c); emit(c,"  %s = extractelement <4 x float> %s, i32 3\n",layerf,uv);
                            const char *layer=newtmp(c); emit(c,"  %s = fptosi float %s to i32\n",layer,layerf);
                            tex_sample_grad_cube_array_used[ti_kind]=1;
                            emit(c,"  %s = call { %s, i8 } @air.sample_texture_cube_array_grad.%s(%%struct._texture_cube_array_t addrspace(1)* %s, %%struct._sampler_t addrspace(2)* %s, <3 x float> %s, i32 %s, <3 x float> %s, <3 x float> %s, float 0.0, i32 0)\n",r,vec,suf,tname,sname,dir,layer,dx,dy);
                        } else if(tp->ty.tex_cube){
                            if(uvw!=3) die(0,"cube SampleGrad direction must be a float3");
                            tex_sample_grad_cube_used[ti_kind]=1;
                            emit(c,"  %s = call { %s, i8 } @air.sample_texture_cube_grad.%s(%s addrspace(1)* %s, %%struct._sampler_t addrspace(2)* %s, <3 x float> %s, <3 x float> %s, <3 x float> %s, float 0.0, i32 0)\n",r,vec,suf,tex_air_type(&tp->ty),tname,sname,uv,dx,dy);
                        } else if(tp->ty.tex_dim==3){
                            if(uvw!=3) die(0,"texture3d SampleGrad coordinate must be a float3");
                            tex_sample_grad_3d_used[ti_kind]=1;
                            emit(c,"  %s = call { %s, i8 } @air.sample_texture_3d_grad.%s(%s addrspace(1)* %s, %%struct._sampler_t addrspace(2)* %s, <3 x float> %s, <3 x float> %s, <3 x float> %s, float 0.0, i1 true, <3 x i32> zeroinitializer, i32 0)\n",r,vec,suf,tex_air_type(&tp->ty),tname,sname,uv,dx,dy);
                        } else if(tp->ty.tex_array&&tp->ty.tex_dim==2){
                            if(uvw!=3) die(0,"texture2d array SampleGrad coordinate must be a float3");
                            ...
                            const char *layer=newtmp(c); emit(c,"  %s = fptosi float %s to i32\n",layer,lf);
                            tex_sample_grad_2d_array_used[ti_kind]=1;
                            emit(c,"  %s = call { %s, i8 } @air.sample_texture_2d_array_grad.%s(%s addrspace(1)* %s, %%struct._sampler_t addrspace(2)* %s, <2 x float> %s, i32 %s, <2 x float> %s, <2 x float> %s, float 0.0, i1 true, <2 x i32> zeroinitializer, i32 0)\n",r,vec,suf,tex_air_type(&tp->ty),tname,sname,uv2,layer,dx2,dy2);
                        } else {
                            if(tp->ty.tex_dim==1) die(0,"texture1d SampleGrad is not supported by Metal AIR");
                            if(uvw!=2) die(0,"texture SampleGrad coordinate must be a float2");
                            tex_sample_grad_used[ti_kind]=1;
                            emit(c,"  %s = call { %s, i8 } @air.sample_texture_2d_grad.%s(%s addrspace(1)* %s, %%struct._sampler_t addrspace(2)* %s, <2 x float> %s, <2 x float> %s, <2 x float> %s, float 0.0, i1 true, <2 x i32> zeroinitializer, i32 0)\n",r,vec,suf,tex_air_type(&tp->ty),tname,sname,uv,dx,dy);
                        }
                        const char *v=newtmp(c); emit(c,"  %s = extractvalue { %s, i8 } %s, 0\n",v,vec,r);
                        if(tp->ty.tex_elt==T_HALF){ const char *w=newtmp(c); emit(c,"  %s = fpext <4 x half> %s to <4 x float>\n",w,v); v=w; }
                        *k=VK_F32; c->rvw=4; return v;
                    ...
                    if(!strcmp(e->name,"sample")||!strcmp(e->name,"Sample")||!strcmp(e->name,"SampleLevel")){
                        ...
                        char sname[64]; snprintf(sname,sizeof sname,"%%_%s",e->args[0]->name);
                        if(tp->ty.tex_cube&&tp->ty.tex_array){
                            if(uvw!=4) die(0,"cube-array sample coordinate must be a float4");
                            const char *dir=newtmp(c); emit(c,"  %s = shufflevector <4 x float> %s, <4 x float> undef, <3 x i32> <i32 0, i32 1, i32 2>\n",dir,uv);
                            const char *layerf=newtmp(c); emit(c,"  %s = extractelement <4 x float> %s, i32 3\n",layerf,uv);
                            const char *layer=newtmp(c); emit(c,"  %s = fptosi float %s to i32\n",layer,layerf);
                            tex_sample_cube_array_used[ti_kind]=1;
                            emit(c,"  %s = call { %s, i8 } @air.sample_texture_cube_array.%s(%%struct._texture_cube_array_t addrspace(1)* %s, %%struct._sampler_t addrspace(2)* %s, <3 x float> %s, i32 %s, i1 %s, float %s, float %s, i32 0)\n",r,vec,suf,tname,sname,dir,layer,lod_flag,lod,fconst(c,0.0));
                        } else if(tp->ty.tex_cube){
        ...
        for(size_t i=0;i<c->prog->nfuncs;i++){
            ...
            if(strcmp(cf->name,e->name)) continue;
            int vis=0; for(size_t q=0;q<cf->nparams;q++) if(!shared_array_type(&cf->params[q].ty)) vis++;
            if(e->nargs> (size_t)vis) continue;
            int rk=0, bad=0;
            if(e->nargs<(size_t)vis){ int no_def=0; size_t seen=0;
                for(size_t q=0;q<cf->nparams;q++){
                    if(shared_array_type(&cf->params[q].ty)) continue;
        ...
        if(f){
            ...
            int fvis=0; for(size_t q=0;q<f->nparams;q++) if(!shared_array_type(&f->params[q].ty)) fvis++;
            ...
            for(size_t i=0;i<f->nparams;i++){ Param *p=&f->params[i];
                if(shared_array_type(&p->ty)) continue; /* synthetic threadgroup storage is not an AIR argument */
                Expr implicit={0};
                ...
                if(p->ty.array_n){
                    const char *pv=NULL; int already_ptr=0;
                    if(arge->kind==E_IDENT){
                        int oi; RKind or=resolve(c,arge->name,&oi);
                        if(or==R_LOCAL && c->locs[oi].an) pv=c->locs[oi].slot;
                        else if(or==R_PTR && c->fn->params[oi].ty.array_n && !shared_array_type(&c->fn->params[oi].ty)){
                            char *av=malloc(128); snprintf(av,128,"%%_%s",c->fn->params[oi].name); pv=av; already_ptr=1;
                        }
                    }
                    if(!pv){ LInfo ali={0}; pv=gen_lval(c,arge,&ali,0); }
                    if(!already_ptr){
                        Type at; expr_type_of(c,arge,&at); char aty[160]; aggregate_ll(aty,sizeof aty,&at);
                        char *first=newtmp(c); emit(c,"  %s = getelementptr inbounds %s, %s* %s, i64 0, i64 0\n",first,aty,aty,pv); pv=first;
                    }
                    char elt[128]; array_elem_ll(elt,sizeof elt,&p->ty);
                    o+=snprintf(args+o,sizeof args-o,"%s addrspace(%d)* %s",elt,p->ty.as,pv);
                } else if(p->ty.kind==T_STRUCT && !p->ty.is_ptr){
...
static char *gen_lval(CG *c, Expr *e, LInfo *li, int mark){
    ...
    if(e->kind==E_INDEX){
        ...
        /* local and struct-field arrays: a[i], s.v[i], buf[i].v[j] */
        {
            ...
            if(is_arr){
                ...
                if(li2.an>0){ ValKind ik; const char *iv=gen_rval(c,e->rhs,&ik);
                    ...
                    char elt[128]; type_ll(elt,sizeof elt,li2.tk,li2.sname,li2.vecn);
                    char asp[32]; if(li2.as) snprintf(asp,sizeof asp," addrspace(%d)",li2.as); else asp[0]='\0';
                    if(li2.am>0){
        ...
        if(e->operand->kind==E_IDENT){
            RKind rr=resolve(c,e->operand->name,&pi);
            if(rr!=R_PTR) die(0,"subscript of non-pointer");
            Param *sp=&c->fn->params[pi];
            if(sp->ty.array_n && shared_array_type(&sp->ty)){
            ...
            Param *sp=&c->fn->params[pi]; if(!shared_array_type(&sp->ty)||!sp->ty.array_m) die(0,"too many subscripts");
...
static void emit_local_array_initializer(CG *c, const char *slot, const char *arrty, const char *elt, Type bty, Expr *init){
    ...
    for(size_t i=0;i<init->nargs;i++){
        ...
        if(bty.kind==T_STRUCT){
            ValKind k; const char *v=gen_rval(c,init->args[i],&k);
            if(!c->rstruct||strcmp(c->rstruct,bty.struct_name)) die(init->line,"struct type mismatch in array initializer");
            char sn[128]; snprintf(sn,sizeof sn,"%%struct.%s",bty.struct_name);
            int al=4; StructDef *sd=find_struct(c->prog,bty.struct_name); if(sd) struct_layout(sd,&al);
            emit(c,"  store %s %s, %s* %s, align %d\n",sn,v,sn,ep,al);
        } else {
            ValKind k; const char *v=gen_rval(c,init->args[i],&k);
...
static void gen_stmt(CG *c, Stmt *s){
    ...
    case S_DECL:{ Type bty=bind_ty(c->fn,s->ty); TypeKind kk=bty.kind;
        ...
        if(kk==T_STRUCT && !bty.array_n){ StructDef *sd=find_struct(c->prog,bty.struct_name);
        ...
        if(bty.array_n){
            ...
            else type_ll(elt,sizeof elt,kk,bty.struct_name,bty.vecn);
            char arrty[160];
            ...
            c->locs=realloc(c->locs,(c->nlocs+1)*sizeof(Loc));
            c->locs[c->nlocs++]=(Loc){s->name,slot,kk,bty.struct_name,bty.matn?0:bty.vecn,bty.matn,bty.matm,s->is_const,bty.array_n,bty.array_m};
...
static int meta_count(const Function *fn){
    int n=0, ex=explicit_fn(fn); for(size_t i=0;i<fn->nparams;i++) if(!shared_array_type(&fn->params[i].ty)) n++;
...
static void fn_ptr_str(Function *fn,char *buf,size_t n){
    ...
    for(size_t i=0;i<fn->nparams;i++){ Param *p=&fn->params[i]; if(shared_array_type(&p->ty))continue; if(emitted++)o+=snprintf(buf+o,n-o,", ");
        ...
        else if(p->ty.array_n){ char elt[128]; array_elem_ll(elt,sizeof elt,&p->ty); o+=snprintf(buf+o,n-o,"%s addrspace(%d)*",elt,p->ty.as); }
...
static void stage_ptr_str(Function *fn,char *buf,size_t n){
    ...
    for(size_t i=0;i<fn->nparams;i++){ Param *p=&fn->params[i];
        if(p->ty.kind==T_STRUCT && !p->ty.is_ptr && !p->ty.array_n && fn->stage!=ST_NONE){
        ...
        char pl[64]; if(p->ty.array_n) array_elem_ll(pl,sizeof pl,&p->ty); else type_ll(pl,sizeof pl,p->ty.kind,p->ty.struct_name,p->ty.vecn);
        if(emitted++)o+=snprintf(buf+o,n-o,", ");
        if(p->ty.kind==T_TEXTURE) o+=snprintf(buf+o,n-o,"%s addrspace(1)*",tex_air_type(&p->ty));
        else if(p->ty.kind==T_SAMPLER) o+=snprintf(buf+o,n-o,"%%struct._sampler_t addrspace(2)*");
        else if(p->ty.array_n) o+=snprintf(buf+o,n-o,"%s addrspace(%d)*",pl,p->ty.as);
...
static void emit_kernel_meta(Meta *m, const Program *prog, Function *fn, KernelMeta *km,
    ...
    for(int a=0;a<(int)fn->nparams;a++){ Param *p=&fn->params[a]; if(shared_array_type(&p->ty))continue;
...
void emit_air(FILE *out, Program *prog){
    g_curprog=prog;
    memset(builtin_used,0,sizeof builtin_used); memset(atomic_add_used,0,sizeof atomic_add_used); memset(atomic_logic_used,0,sizeof atomic_logic_used); reversebits_used=0; tex_dims_2d_used=0;
    ...
    for(size_t fi=0;fi<prog->nfuncs;fi++) for(size_t pi=0;pi<prog->funcs[fi].nparams;pi++){
        Param *p=&prog->funcs[fi].params[pi]; if(!shared_array_type(&p->ty))continue;
    ...
    for(volatile size_t fi=0;fi<prog->nfuncs;fi++){
        ...
        int emitted=0;
        for(size_t i=0;i<fn->nparams;i++){ Param *p=&fn->params[i];
            if(shared_array_type(&p->ty)) continue; /* shared arrays are module globals, not ABI args */
            if(fn->stage!=ST_NONE && p->ty.kind==T_STRUCT && !p->ty.is_ptr && !p->ty.array_n){
            ...
            else if(p->ty.array_n){ char elt[128]; array_elem_ll(elt,sizeof elt,&p->ty); so+=snprintf(sig+so,sizeof sig-so,"%s addrspace(%d)* nocapture noundef %%_%s",elt,p->ty.as,p->name); }
        ...
        if(fn->stage==ST_NONE) for(size_t pi2=0;pi2<fn->nparams;pi2++){ Param *p=&fn->params[pi2];
            if(p->ty.kind!=T_STRUCT||p->ty.is_ptr||p->ty.array_n) continue;
            StructDef *sd=find_struct(prog,p->ty.struct_name);
    ...
    /* texture intrinsics + the opaque texture/sampler types */
    {
        ...
        if(get_samp_used||tex_read_used[0]||tex_read_used[1]||tex_read_used[2]||tex_read_used[3]||
           ...
           tex_sample_2d_array_used[0]||tex_sample_2d_array_used[1]||tex_sample_2d_array_used[2]||tex_sample_2d_array_used[3]||
           ...
           tex_gather_2d_array_used[0]||tex_gather_2d_array_used[1]||tex_gather_2d_array_used[2]||tex_gather_2d_array_used[3]||tex_dims_2d_used||any_tex_param){
        ...
        if(tex_dims_2d_used){
            fprintf(out,"declare i32 @air.get_width_texture_2d(%%struct._texture_2d_t addrspace(1)* nocapture readonly, i32) local_unnamed_addr\n");
            fprintf(out,"declare i32 @air.get_height_texture_2d(%%struct._texture_2d_t addrspace(1)* nocapture readonly, i32) local_unnamed_addr\n");
            fprintf(out,"declare i32 @air.get_num_mip_levels_texture_2d(%%struct._texture_2d_t addrspace(1)* nocapture readonly) local_unnamed_addr\n");
        }
        ...
        for(int i=0;i<4;i++){
            ...
            if(tex_sample_cube_array_used[i]) fprintf(out,"declare { %s, i8 } @air.sample_texture_cube_array.%s(%%struct._texture_cube_array_t addrspace(1)* nocapture readonly, %%struct._sampler_t addrspace(2)* nocapture readonly, <3 x float>, i32, i1, float, float, i32) local_unnamed_addr\n",vecs[i],sufs[i]);
            if(tex_sample_grad_used[i]) fprintf(out,"declare { %s, i8 } @air.sample_texture_2d_grad.%s(%%struct._texture_2d_t addrspace(1)* nocapture readonly, %%struct._sampler_t addrspace(2)* nocapture readonly, <2 x float>, <2 x float>, <2 x float>, float, i1, <2 x i32>, i32) local_unnamed_addr\n",vecs[i],sufs[i]);
            ...
            if(tex_sample_grad_cube_array_used[i]) fprintf(out,"declare { %s, i8 } @air.sample_texture_cube_array_grad.%s(%%struct._texture_cube_array_t addrspace(1)* nocapture readonly, %%struct._sampler_t addrspace(2)* nocapture readonly, <3 x float>, i32, <3 x float>, <3 x float>, float, i32) local_unnamed_addr\n",vecs[i],sufs[i]);
